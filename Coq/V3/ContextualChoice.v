(** ContextualChoice.v -- generated genealogies for authoritative v3 CCP.

    Formalizes Definitions 8.1, 8.3, 8.4 and the structural closure/invariant
    content of Proposition 8.2 and Theorem 8.5 at a representation-independent
    interface. CCP is an internal admissibility discipline, not an ambient
...
Proof.
  refine {| ccp_evidence_map := fun a =>
            ...
            ccp_queries := fun n a =>
              ccp_queries F (ccp_modulus G n (ccp_evidence_map F a)) a
              ++ ccp_queries G n (ccp_evidence_map F a);
            ccp_modulus := fun n a =>
              ccp_modulus F (ccp_modulus G n (ccp_evidence_map F a)) a |}.
  intro a.
  rewrite (ccp_commutes G (ccp_evidence_map F a)).
  rewrite (ccp_commutes F a). reflexivity.
Defined.
...
Proof. intros. unfold ccp_query_count. simpl. apply app_length. Qed.
...
Proof.
  ...
  intro a. simpl.
  rewrite (ccp_commutes F a), (ccp_commutes G a). reflexivity.
Defined.
...
Proof.
  ...
  - split.
    + intros A B C DA DB DC UA UB UC F G. exact (compose_ccp G F).
    + intros A B C DA DB DC UA UB UC F G. exact (product_ccp F G).
Qed.
...
Section CCPDevelopment.
  ...
  Theorem ccp_claims_are_genealogically_accounted :
    ObeysCCP -> forall x, Claimed x -> Primitive x \/ AdmissibleOutput x.
  Proof. intros [Hclaims Hlocal] x Hx. now apply Hclaims. Qed.

  Theorem ccp_local_to_global_requires_interface :
    ObeysCCP -> forall x, LocalPromotion x -> PromotionJustified x.
  Proof. intros [Hclaims Hlocal] x Hx. now apply Hlocal. Qed.
End CCPDevelopment.
...
Section GeneratedUniverse.
  ...
  | generated_primitive : forall x, Primitive x -> Generated x
  | generated_nullary : forall out, Step0 out -> Generated out
  | generated_unary : forall x out, Generated x -> Step1 x out -> Generated out
  ...
  Theorem generated_least (P : Obj -> Prop) :
    ClosedFamily P -> forall x, Generated x -> P x.
  Proof.
  ...
  Proof.
    intros Hprim H0 H1 H2. apply generated_least.
    split; [exact Hprim|]. split; [exact H0|]. split; assumption.
  Qed.
