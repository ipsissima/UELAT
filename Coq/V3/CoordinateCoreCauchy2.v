(** CoordinateCoreCauchy2.v -- completion step for admissible dual coordinates.
    ...
    The dense-core coordinate functional is 1-Lipschitz and therefore maps
    every fast core name to a fast Cauchy real-value sequence.
*)
...
Section Completion.
  ...
  Definition core_sub_code (p q : core_code B) : core_code B :=
    core_add B p (core_neg_code q).
  ...
  Lemma core_sub_code_sound : forall p q,
    core_decode (core_sub_code p q)
      = cb_sub B (core_decode p) (core_decode q).
  Proof.
  ...
  Proof.
    intro p. unfold core_neg_code. rewrite direct_core_value_scale, Q2R_opp.
  ...
  Theorem coordinate_value_stage_fast : forall nu m n,
    n <= m ->
    Rabs (coordinate_value_stage nu m - coordinate_value_stage nu n)
      <= dyadic n.
  Proof.
    intros nu m n Hnm. unfold coordinate_value_stage.
    eapply Rle_trans; [apply direct_core_value_lipschitz|].
    apply core_stage_fast. exact Hnm.
  Qed.

  Record RealCauchyValueName := {
    rcv_stage : nat -> R;
    rcv_fast : forall m n,
      n <= m -> Rabs (rcv_stage m - rcv_stage n) <= dyadic n
  }.
      ...
      forall n,
        Rabs
          (coordinate_value_stage (core_named_name x) n
           - coordinate_value_stage (core_named_name y) n)
        <= 2 * dyadic n.
  Proof.
