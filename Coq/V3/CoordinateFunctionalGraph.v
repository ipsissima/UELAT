(** CoordinateFunctionalGraph.v -- converse coordinate-to-functional graph on the dense rational span.

    Admissible finite coordinate inequalities define a unique bounded
    rational-linear functional on the normalized rational span. The remaining
    analytic step is effective extension by continuity to the Banach carrier.
*)
...
From UELAT.V3 Require Import
  ...
  DualBallCoordinates CoordinateDualBall.
...
Section SpanFunctional.
  ...
  Definition span_value (ts : list RationalCoordinateTerm) : R :=
    coordinate_sum (cdb_coordinates B a) ts.
  ...
  Fixpoint negate_terms (ts : list RationalCoordinateTerm) :
    ...
    match ts with [] => [] | (q,i) :: rest => (-q,i) :: negate_terms rest end.

  Lemma scale_neg_core_term : forall q i,
    core_decode (core_scale B (-q) (normalized_core_code B (core_enum B i)))
      = cb_neg B (core_decode (core_scale B q (normalized_core_code B (core_enum B i)))).
  Proof.
    intros q i. repeat rewrite core_scale_sound.
    unfold cb_neg. rewrite Q2R_opp, cb_scale_assoc.
    replace ((-1) * Q2R q)%R with (- Q2R q)%R by ring. reflexivity.
  Qed.
  ...
  Definition subtract_terms (xs ys : list RationalCoordinateTerm) : list RationalCoordinateTerm :=
    xs ++ negate_terms ys.

  Lemma span_vector_subtract : forall xs ys,
    span_vector (subtract_terms xs ys) = cb_sub B (span_vector xs) (span_vector ys).
  Proof.
  ...
  Proof. intros xs ys. unfold subtract_terms. rewrite span_value_append, span_value_negate. ring. Qed.

  Lemma admissible_span_bound : forall ts,
    Rabs (span_value ts) <= cb_norm B (span_vector ts).
  Proof. intro ts. unfold span_value, span_vector. exact (cdb_admissible B a ts). Qed.
  ...
  Proof.
    ...
    pose proof (Rabs_pos (span_value xs - span_value ys)) as Habs0. lra.
  Qed.
  ...
  Proof.
    intros x r s [ts [Hx Hr]] [us [Hx' Hs]]. subst r s.
    apply coordinate_value_well_defined. now rewrite <- Hx, <- Hx'.
  Qed.

  Theorem coordinate_span_graph_bounded : forall x r,
    CoordinateSpanGraph x r -> Rabs r <= cb_norm B x.
  Proof. intros x r [ts [Hx Hr]]. subst x r. apply admissible_span_bound. Qed.

  Definition scale_terms (c : Q) (ts : list RationalCoordinateTerm) : list RationalCoordinateTerm :=
  ...
  Proof.
    ...
    - ring.
    - unfold span_value in *; simpl in *. rewrite Q2R_mult, IH. ring.
  Qed.
  ...
  Proof.
    ...
    - simpl. rewrite span_vector_nil. symmetry. apply cb_scale_zero_vector.
    - unfold span_vector in *; simpl in *.
      repeat rewrite core_add_sound. repeat rewrite core_scale_sound.
      rewrite Q2R_mult, IH, cb_scale_add_vectors, cb_scale_assoc. reflexivity.
  Qed.
  ...
  Theorem coordinate_span_graph_generator : forall i,
    CoordinateSpanGraph (normalized_core B i) (cdb_coordinates B a i).
  Proof.
