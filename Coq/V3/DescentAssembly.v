(** DescentAssembly.v -- represented-limit and resource assembly for authoritative v3 Theorem 7.4.

    Constructs an actual fast-Cauchy represented name directly from selected
    finite approximants and a precision schedule, without querying an
    independently supplied target name.  It also packages the size,
    verification and source-lookahead consequences of the H1/H6/H7 resource
    hypotheses.
*)
...
Lemma dyadic_antitone : forall n m, n <= m -> dyadic m <= dyadic n.
Proof.
  intros n m Hnm. induction Hnm.
  - lra.
  - eapply Rle_trans; [apply dyadic_step_le|exact IHHnm].
Qed.
