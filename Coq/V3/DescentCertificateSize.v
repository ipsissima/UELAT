(** DescentCertificateSize.v -- full certificate-size comparison for authoritative v3 Theorem 7.4.

    H1H7Descent.v bounds accumulated proof payload. The manuscript certificate
    also contains the ordinary finite approximant encoding at the selected
    level. This module adds that term explicitly and proves a single
    constant-factor comparison with B_{mu(s)}.
*)
...
Section Size.
  ...
  Proof.
    intro s.
    unfold size_denominator, precision_proof_bits, proof_factor.
  ...
  Proof.
    ...
    unfold precision_certificate_bits, total_factor in *.
    nia.
  Qed.
  ...
  Proof.
    ...
    pose proof (h_cnum_pos H).
    pose proof (h_Cden_pos H).
    nia.
  Qed.
End Size.
