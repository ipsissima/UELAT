(** DyadicVanishing.v -- quantitative tail lemmas for strict-slack search and
    ...
    Besides eventual smallness of 2^{-n}, this file records the monotonicity
    and fixed-shift estimates repeatedly used when turning independent finite
    approximations into a fast Cauchy name.
*)
...
Lemma dyadic_as_power : forall n,
  dyadic n = (/ 2) ^ n.
Proof.
  induction n as [|n IH].
  - reflexivity.
...
Lemma half_power_nonnegative : forall n,
  0 <= (/ 2) ^ n.
Proof.
  intro n. apply pow_le. left. apply Rinv_0_lt_compat. lra.
Qed.

Lemma dyadic_step_le : forall n,
  dyadic (S n) <= dyadic n.
Proof.
  intro n. simpl.
  pose proof (dyadic_nonnegative n).
  lra.
Qed.
...
Proof.
  intros eps Heps.
  destruct (dyadic_eventually_below (eps / 2)) as [n Hn]; [lra|].
  exists n. lra.
Qed.
...
Proof.
  intros eps Heps.
  destruct (dyadic_eventually_below (eps / 4)) as [n Hn]; [lra|].
  exists n. lra.
Qed.
