(** EffectiveCoordinateFunctional2.v -- effective completion of an admissible coordinate point to a represented real-valued functional.

    Rational dyadic coordinate names are read at enough precision to compensate
    for the rational rescaling p=c_p z_i. Reading a source name two stages ahead
    yields a rational fast Cauchy name of the functional value.
*)
...
From UELAT.V3 Require Import
  ...
  SearchableCore DualBallCoordinates CoordinateDualBall
...
Section EffectiveCoordinates.
  ...
  Record EffectiveCoordinatePoint := {
    ...
    ecp_approx_sound : forall i n,
      Rabs (Q2R (ecp_approx i n) - cdb_coordinates B ecp_point i) <= dyadic n
  }.

  Variable a : EffectiveCoordinatePoint.

  Definition scale_precision_test (p : core_code B) (n k : nat) : bool :=
  ...
  Definition scale_precision (p : core_code B) (n : nat) : nat :=
    first_true_index (scale_precision_test p n) (scale_precision_eventually p n).

  Lemma scale_precision_valid : forall p n,
    Q2R (core_scale_factor B p) * dyadic (scale_precision p n) < dyadic n.
  Proof.
    intros p n.
    pose proof (first_true_valid (scale_precision_test p n)
      (scale_precision_eventually p n)) as Htest.
    apply qltb_true_iff in Htest.
    pose proof (Qlt_Rlt _ _ Htest) as Hreal.
    rewrite Q2R_mult, !qdyadic_real in Hreal. exact Hreal.
  Qed.

  Definition core_value_rational_approx (p : core_code B) (n : nat) : Q :=
    core_scale_factor B p * ecp_approx a (core_index S p) (scale_precision p n).

  Theorem core_value_rational_approx_sound : forall p n,
    Rabs (Q2R (core_value_rational_approx p n)
       - direct_core_value S (ecp_point a) p) < dyadic n.
  Proof.
    intros p n. unfold core_value_rational_approx, direct_core_value.
    rewrite Q2R_mult.
    set (c := Q2R (core_scale_factor B p)).
    set (z := Q2R (ecp_approx a (core_index S p) (scale_precision p n))).
    set (v := cdb_coordinates B (ecp_point a) (core_index S p)).
    replace (c*z - c*v) with (c*(z-v)) by ring.
    rewrite Rabs_mult.
    assert (Hc : 0 < c) by (unfold c; apply core_scale_factor_real_positive).
    rewrite Rabs_pos_eq by lra.
    pose proof (ecp_approx_sound a (core_index S p) (scale_precision p n)) as Hz.
    fold z v in Hz.
    pose proof (scale_precision_valid p n) as Hscale. fold c in Hscale.
    eapply Rle_lt_trans.
    - apply Rmult_le_compat_l; [lra|exact Hz].
    - exact Hscale.
  Qed.

  Definition represented_functional_qstage (nu : CoreFastName B) (n : nat) : Q :=
    let s := S (S n) in core_value_rational_approx (core_stage nu s) s.

  Lemma three_shifted_dyadics_le : forall n,
    3 * dyadic (S (S n)) <= dyadic n.
  Proof. intro n. simpl. pose proof (dyadic_nonnegative n). nra. Qed.

  Theorem represented_functional_qstage_fast : forall nu m n,
    n <= m ->
    Rabs (Q2R (represented_functional_qstage nu m)
       - Q2R (represented_functional_qstage nu n)) <= dyadic n.
  Proof.
    intros nu m n Hnm.
    ...
    assert (Han' : Rabs (Fn - Q2R (core_value_rational_approx pn sn)) < dyadic sn).
    ...
    assert (Htri :
      Rabs (Q2R (core_value_rational_approx pm sm)
    ...
    { replace
        (Q2R (core_value_rational_approx pm sm)
         - Q2R (core_value_rational_approx pn sn))
        with ((Q2R (core_value_rational_approx pm sm)-Fm)
  ...
  Record RationalRealName := {
    rrn_stage : nat -> Q;
    rrn_fast : forall m n,
      n <= m -> Rabs (Q2R (rrn_stage m) - Q2R (rrn_stage n)) <= dyadic n
  }.

  Definition represented_functional_name (nu : CoreFastName B) : RationalRealName :=
    {| rrn_stage := represented_functional_qstage nu;
       rrn_fast := represented_functional_qstage_fast nu |}.

  Theorem admissible_effective_coordinates_define_real_name :
    forall nu : CoreFastName B, RationalRealName.
  Proof. exact represented_functional_name. Qed.
