(** EpsilonPrecision.v -- rational epsilon selector for authoritative Theorem 7.4.

    The core descent development is indexed by a dyadic precision exponent s.
    The manuscript states Theorem 7.4 for rational 0 < epsilon <= 1. This file
    bridges the two by searching for the first s with 4*2^{-s}<epsilon.
*)
...
Proof.
  intros eps Heps. unfold epsilon_precision.
...
Proof.
  ...
  unfold epsilon_stage_test in H.
  apply qltb_true_iff in H.
  pose proof (Qlt_Rlt _ _ H) as HR.
  rewrite Q2R_mult, qdyadic_real in HR.
  change (Q2R (4 : Q)) with 4%R in HR.
  exact HR.
Qed.
...
Proof.
  intros eps Heps.
  pose proof (epsilon_precision_dyadic_bound eps Heps).
