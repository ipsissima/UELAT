(** EvidenceReindexing.v -- manuscript Proposition 4.6.

    Certificate systems can be reindexed to answer a target tolerance eps by
    using their answer at eps/2^k.  This changes no represented analytic point
    and acts identically on distance arrows, but changes the chosen object-level
    evidence schedule.  The resulting R_k form the precision-reindexing monoid
    used in the non-faithfulness theorem.
*)
...
Proof.
  induction k; simpl; intros eps Heps; try lra.
  specialize (IHk eps Heps). lra.
Qed.

Lemma shrink_nonnegative : forall k eps, 0 <= eps -> 0 <= shrink k eps.
Proof.
  intros k eps Heps.
  destruct Heps as [Heps|Heps].
  - left. now apply shrink_pos.
  - subst eps. induction k; simpl; lra.
Qed.

Lemma shrink_le_original : forall k eps,
  0 <= eps -> shrink k eps <= eps.
Proof.
  induction k; simpl; intros eps Heps.
  - lra.
  - pose proof (IHk eps Heps) as Hle.
    pose proof (shrink_nonnegative k eps Heps) as Hnonneg.
    lra.
Qed.
...
Proof.
  simpl. intros k eps Heps.
  pose proof (shrink_pos k eps Heps). lra.
Qed.
...
Section Reindex.
  ...
  Definition reindex_object (k : nat)
      (a : EvidenceObject E) : EvidenceObject E :=
    {| ev_name := ev_name E a;
       ev_system := reindex_system k (ev_system E a) |}.
  ...
  Proof.
    ...
    pose proof (shrink_pairwise_distinct k l eps Heps Hneq).
    nra.
  Qed.
