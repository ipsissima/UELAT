(** EvidenceTransport.v -- authoritative Theorem 4.2.

    Contains the analytic Lipschitz estimate and checker-level evidence-local
    constructor. The canonical lift asks the source certificate system once at
    eps/(3 max(1,Lambda)) and calls the finite-code compiler once at defect
    eps/3; it makes no independent target-name certification query.
*)
...
Section AnalyticTransport.
  ...
  Theorem transported_approximation_bound
      (x : carrier X) (p : carrier X) (q : carrier Y) (r eta : R) :
    distance x p <= r ->
    distance (T p) q <= eta ->
  ...
  Proof.
    ...
    - pose proof (T_lipschitz x p) as Hlip. nra.
  Qed.

  Theorem transported_distance_bound (x x' : carrier X) (r : R) :
    distance x x' <= r -> distance (T x) (T x') <= Lambda * r.
  Proof. intro Hsrc. pose proof (T_lipschitz x x') as Hlip. nra. Qed.
...
Section CheckerLevelLift.
  ...
  Definition max_one_lambda : R := if Rle_dec Lambda 1 then 1 else Lambda.

  Lemma max_one_lambda_ge_one : 1 <= max_one_lambda.
  Proof. unfold max_one_lambda. destruct (Rle_dec Lambda 1); simpl; lra. Qed.

  Lemma lambda_le_max_one : Lambda <= max_one_lambda.
  Proof. unfold max_one_lambda. destruct (Rle_dec Lambda 1); simpl; lra. Qed.

  Lemma max_one_lambda_pos : 0 < max_one_lambda.
  Proof. pose proof max_one_lambda_ge_one. lra. Qed.

  Definition source_tolerance (eps : R) : R := eps / (3 * max_one_lambda).
  Definition compiler_tolerance (eps : R) : R := eps / 3.

  Lemma source_tolerance_pos : forall eps,
    0 < eps -> 0 < source_tolerance eps.
  Proof.
    intros eps Heps. unfold source_tolerance.
  ...
  Lemma source_part_below_third : forall eps r,
    0 < eps -> 0 <= r -> r < source_tolerance eps ->
    Lambda * r < eps / 3.
  Proof.
    ...
    { unfold source_tolerance. field_simpl. nra. }
  ...
  Definition lift_certificate_system
      (C : EvidenceLocalCompiler) (nu : name X)
      (c : CertificateSystem EX nu) : CertificateSystem EY (Tname nu).
  Proof.
    intros eps Heps.
    ...
    destruct (compile_app C nu (cert_code EX src) (cert_bound EX src)
                (cert_evidence EX src) eta (cert_accepted EX src)
                (Rlt_le _ _ (compiler_tolerance_pos eps Heps))) as [wy Hwy].
    refine {| certificate_at_record :=
                {| cert_code := compile_code C (cert_code EX src) eta;
                   ...
                   cert_evidence := wy; cert_accepted := Hwy |};
              certificate_at_strict := _ |}.
    - pose proof (cert_bound_nonnegative EX src).
      pose proof (compiler_tolerance_pos eps Heps). nra.
    - pose proof (certificate_at_strict EX src) as Hsrc.
      ...
      unfold eta, compiler_tolerance. lra.
  Defined.

  Definition lift_distance
      (C : EvidenceLocalCompiler) {a b : EvidenceObject EX}
  ...
  Proof.
    destruct (compile_dist C (ev_name EX a) (ev_name EX b)
    ...
    refine {| arrow_bound := Lambda * arrow_bound EX f;
              ...
              arrow_witness := wy; arrow_accepted := Hwy |}.
    pose proof (arrow_bound_nonnegative EX f). nra.
  Defined.

  Theorem qualitative_local_transport_saturation (C : EvidenceLocalCompiler) :
    (forall nu c, exists out, out = lift_certificate_system C nu c)
    /\ (forall a b (f : EvidenceArrow EX a b),
       exists out, out = lift_distance C f).
  Proof. split; intros; eexists; reflexivity. Qed.
