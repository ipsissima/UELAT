(** GenericSlackCertification.v -- manuscript Proposition 2.5.
    ...
    The proposition is conditional on a computable metric presentation whose
    checker may read the supplied Cauchy names.  The exact effective ingredient
    needed is a rational upper-enclosure routine for finite-stage distances.
    This module states that ingredient explicitly and proves that every true
    strict rational bound is eventually recognized by a finite rational test.
*)
...
Lemma qdyadic_real : forall n,
  Q2R (qdyadic n) = dyadic n.
Proof.
  induction n as [|n IH].
  - reflexivity.
...
Lemma qltb_true_iff : forall a b,
  qltb a b = true <-> (a < b)%Q.
Proof.
  intros a b. unfold qltb.
...
Section DistanceCertification.
  ...
  Proof.
    ...
    pose proof (ems_upper_sound E
      (ems_stage E nu n) (ems_stage E mu n) n) as Hupper.
    rewrite distance_symmetric with
      (x := decode_name mu) (y := ems_stage E mu n) in Hmu.
    eapply Rle_lt_trans.
  ...
  Proof.
    ...
    destruct (dyadic_eventually_below ((Q2R q - dtrue) / 8) ltac:(lra))
      as [n Hsmall].
    assert (Hfinite :
    ...
    { pose proof (ems_stage_tail E nu n) as Hnu.
      ...
      - eapply Rle_trans.
        ...
        + rewrite distance_symmetric with
            (x := ems_stage E nu n) (y := decode_name nu).
          unfold dtrue. lra. }
    pose proof (ems_upper_precision E
      (ems_stage E nu n) (ems_stage E mu n) n) as Hprec.
    assert (Hreal :
  ...
  Proof.
    intros. unfold distance_slack_stage.
    apply semidecidable_slack_search_valid.
  Qed.
...
Section ApproximationCertification.
  ...
  Definition approximation_stage_test
    ...
    qltb
      (ems_upper E (ems_stage E nu n) (decode p) n + qdyadic n)
      q.
  ...
  Proof.
    ...
    assert (Hgap : 0 < Q2R q - dtrue) by (unfold dtrue; lra).
    destruct (dyadic_eventually_below ((Q2R q - dtrue) / 4) ltac:(lra))
      as [n Hsmall].
    assert (Hfinite :
      distance (ems_stage E nu n) (decode p) <= dtrue + dyadic n).
    { pose proof (ems_stage_tail E nu n) as Htail.
      eapply Rle_trans.
      - apply distance_triangle with (y := decode_name nu).
      - rewrite distance_symmetric with
    ...
    assert (Hreal :
      Q2R (ems_upper E (ems_stage E nu n) (decode p) n + qdyadic n)
        < Q2R q).
    { rewrite Q2R_plus, qdyadic_real.
      unfold dtrue in *. lra. }
    exists n.
    apply qltb_true_iff.
    now apply Rlt_Qlt.
  ...
  Proof.
    intros. unfold approximation_slack_stage.
    apply semidecidable_slack_search_valid.
  Qed.
