(** GeometricPrecisionSchedule.v -- derive the Theorem 7.4 descent name schedule from H4.

    If K*2^{-c} <= 1 and the level error decays as K*2^{-alpha n}, the
    computable schedule mu(s)=1+floor((s+1+c)/alpha) yields an error at most
    2^{-(s+1)}. This supplies the represented-name schedule rather than
    assuming one independently.
*)
...
Definition geometric_precision_schedule (alpha offset s : nat) : nat :=
  S ((s + 1 + offset) / alpha).
