(** GrothendieckEvidence.v -- authoritative Proposition 4.7.

    Formalizes the structural content of the standard Grothendieck construction
    for a strict covariant Cat-valued assignment: strict fibre transport, total
    objects, chosen lifts, split identity/composition laws, and the target-fibre
    factor. No probes-models adjunction is involved.
*)
...
Section Construction.
  ...
  Definition chosen_lift {x y : BObj B}
    ...
    {| lift_target := push_obj f a; lift_target_is_push := eq_refl |}.
  ...
  Record TotalArrow (a b : TotalObject) := {
    ...
    total_arrow_fibre :
      fibre_hom I (total_base b)
        (push_obj total_arrow_base (total_fibre a)) (total_fibre b)
  }.
  ...
  Proof.
    refine {| total_arrow_base := f;
              total_arrow_fibre := fibre_id I y (push_obj f a) |}.
  Defined.
  ...
  Proof.
    intros. exists (opcartesian_factor f g a c h).
    - reflexivity.
  ...
  Proof.
    refine {| split_lift := fun _ _ f a => push_obj f a;
              split_factor := fun _ _ _ f g a c h => opcartesian_factor f g a c h |}.
    - apply push_id.
    - apply push_comp.
  Defined.
End Construction.
