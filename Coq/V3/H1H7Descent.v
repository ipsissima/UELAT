(** H1H7Descent.v -- H1--H7 assembly for authoritative v3 Theorem 7.4.

    The individual numerical/refinement ingredients are placed behind one
    record corresponding to H1--H7 and used to derive a represented limit
    together with genealogy-size, verification, source-lookahead and
    zero-target-query conclusions. The represented-name schedule is derived
    from H4's geometric rate rather than supplied independently.
*)
...
Section FullInterface.
  ...
  Record H1H7Data := {
    ...
    (** H4 -- exact synthesis and geometric scale law. *)
    ...
    (** H5 -- persistent shared genealogy and no generic target recertifier. *)
    ...
    (** H6 -- coefficient-wise encoding and proof payload. *)
    h_beta : nat -> nat;
    ...
    (** H7 -- fixed-arity arithmetic verification. *)
    h_A : nat -> nat;
    ...
    (** Conditional source-query clause. *)
