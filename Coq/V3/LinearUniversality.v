(** LinearUniversality.v -- invariant mathematical core of authoritative Theorem 3.2.

    A 1-norming family of linear coordinates gives a linear evaluation
    representation whose coordinate supremum is exactly the norm, expressed by
    the epsilon characterization of the supremum. The final section records the
    finite-code consequence used by Corollary 3.4.
*)
...
Section IndexedNormingEvaluation.
  ...
  Hypothesis indexed_one_norming : forall x eta,
      0 < eta -> exists j, norm x - eta < Rabs (icoord j x).
