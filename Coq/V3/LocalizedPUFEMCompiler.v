...
Inductive PUFEMRule :=
| UseLocalDefect | ApplyMultiplier | RecordContribution
| AggregatePatch | RecordScaleEstimate | RecordTransition.
...
Definition incidence_contribution (Cinf : R) (d : IncidencePayload) : R :=
  (Cinf^2 + 2 * ip_L_sq d) * ip_delta0_sq d
  + 2 * Cinf^2 * ip_delta1_sq d.
Fixpoint contribution_sum (Cinf : R) (ds : list IncidencePayload) : R :=
  match ds with | [] => 0 | d :: rest => incidence_contribution Cinf d + contribution_sum Cinf rest end.
Definition patch_defect_bound (kappa Cinf : R) (ds : list IncidencePayload) : R :=
  kappa * contribution_sum Cinf ds.

Fixpoint incidence_steps (Cinf : R) (ds : list IncidencePayload) : list (PUFEMRule * PUFEMPayload) :=
  match ds with
  | [] => []
  | d :: rest =>
...
Fixpoint patch_steps (M : nat) : list (PUFEMRule * PUFEMPayload) :=
  match M with | O => [] | S k => (AggregatePatch, StructuralData k) :: patch_steps k end.
...
Lemma patch_steps_length : forall M, length (patch_steps M) = M.
Proof. induction M; simpl; lia. Qed.
Definition one_cover_steps (Cinf : R) (incidences : list IncidencePayload) (M : nat) :=
...
Proof. intros. unfold one_cover_steps. rewrite app_length, incidence_steps_length, patch_steps_length. lia. Qed.
...
Theorem compile_one_cover_persistent : forall H Cinf incidences M,
  exists tail, dag_nodes (compile_one_cover H Cinf incidences M) = dag_nodes H ++ tail.
Proof. intros. unfold compile_one_cover. apply append_steps_persistent. Qed.
...
Proof.
  intros H Cinf incidences M. unfold compile_one_cover.
...
Definition scale_sensitive_incidence (hinv_sq delta0_sq delta1_sq : R) : R :=
