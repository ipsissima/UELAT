...
Record PairwiseDefectDatum := {
  pdd_delta0 : R; pdd_delta1 : R; pdd_L : R;
...
Definition Ldelta0_sq (d : PairwiseDefectDatum) : R := pdd_L d ^ 2 * pdd_delta0 d ^ 2.
Definition sum_delta0_sq ds := rsum (map delta0_sq ds).
Definition sum_delta1_sq ds := rsum (map delta1_sq ds).
Definition sum_Ldelta0_sq ds := rsum (map Ldelta0_sq ds).
...
Lemma defect_square_sums_nonnegative : forall ds,
  0 <= sum_delta0_sq ds /\ 0 <= sum_Ldelta0_sq ds /\ 0 <= sum_delta1_sq ds.
Proof.
  intro ds. unfold sum_delta0_sq, sum_Ldelta0_sq, sum_delta1_sq.
...
Definition manuscript_R (kappa Cinf : R) (ds : list PairwiseDefectDatum) : R :=
  kappa * (Cinf^2 * sum_delta0_sq ds
     + 2 * sum_Ldelta0_sq ds
     + 2 * Cinf^2 * sum_delta1_sq ds).

Record ComponentEvidence (kappa Cinf : R) (ds : list PairwiseDefectDatum) := {
  component_l2_sq : R;
  ...
  component_deriv_bound :
    component_deriv_sq <= 2 * kappa *
      (sum_Ldelta0_sq ds + Cinf^2 * sum_delta1_sq ds)
}.
...
Theorem compiled_localized_defect_is_sound :
  forall kappa Cinf Hkappa HCinf ds E,
    component_l2_sq E + component_deriv_sq E <= manuscript_R kappa Cinf ds.
Proof.
  intros. exact (cld_total_defect_bound
...
(** Corrected current signature: [one_cover_steps] also receives Cinf. *)
Theorem compiled_structural_step_count : forall Cinf ds M,
  length (one_cover_steps Cinf (incidence_payloads ds) M)
    = 3 * length ds + M.
Proof.
  intros Cinf ds M. rewrite one_cover_steps_length, incidence_payload_count.
  reflexivity.
Qed.
