(** NormalizedApproxHBStrong.v -- normalization of the Type-2 epsilon-Hahn--Banach output.

    Given g(v)=||v|| and ||g||<=1+eta, divide both semantic functional and
    rational realizer by 1+eta. The result is contracting, computable on source
    names, and attains ||v||/(1+eta).
*)
...
Section Normalize.
  ...
  Proof.
    ...
    change (Q2R (0 : Q)) with 0%R in H. lra.
  Qed.

  Lemma denomQ_nonzero : ~ denomQ == 0.
  Proof.
    intro Hzero. apply Qeq_eqR in Hzero.
    unfold denomQ in Hzero. rewrite Q2R_plus in Hzero.
    change (Q2R (1 : Q)) with 1%R in Hzero.
    change (Q2R (0 : Q)) with 0%R in Hzero.
    pose proof denomR_positive. unfold denomR in H. lra.
  Qed.
  ...
  Proof.
    ...
    - lra.
    - intro x.
      rewrite Rabs_div. rewrite Rabs_pos_eq by lra.
      ...
      field_simplify; try lra. nra.
    - intros x n.
      rewrite Q2R_div by exact denomQ_nonzero.
      unfold denomQ. rewrite Q2R_plus.
      ...
      pose proof denomR_positive as Hd. unfold denomR in Hd.
      ...
      change (Q2R (0 : Q)) with 0%R in HetaR. nra.
  Defined.

  Theorem normalized_strong_contracting : forall x,
    Rabs (rbf_apply normalized_realized_hb x) <= cb_norm B x.
  Proof. intro x. pose proof (rbf_bounded normalized_realized_hb x). simpl in H. nra. Qed.
  ...
  Theorem normalized_strong_has_realizer : forall x n,
    Rabs
      (Q2R (rbf_realize normalized_realized_hb (core_named_name x) n)
       - rbf_apply normalized_realized_hb (core_named_value x)) <= dyadic n.
