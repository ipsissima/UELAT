(** NormingFamilyStrongBridge.v -- strong effective Step 1 bridge. *)

From Coq Require Import Reals.
From UELAT.V3 Require Import
  CertificateEnrichment ComputableBanach
  RealizedBoundedFunctional ApproximateHahnBanachStrongInterface
  EffectiveNormingFamilyStrong OneNormingStrong LinearUniversality.
...
Import UELAT_V3_OneNormingStrong.
...
Section Bridge.
  ...
  Definition strong_evaluation (x : X) : CandidateIndex -> R :=
    fun j => strong_coord j x.
  ...
  Theorem strong_coord_realizer_correct : forall j x n,
    Rabs
      (Q2R (strong_coord_realizer j (core_named_name x) n)
       - strong_coord j (core_named_value x))
      <= dyadic n.
  Proof.
    intros. unfold strong_coord_realizer, strong_coord.
