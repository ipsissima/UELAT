(** NormingPolar.v -- elementary polar computation behind authoritative Theorem 3.2 Step 3.

    If (u_j) is contracting and 1-norming, then its polar is exactly the closed
    unit ball. This isolates the elementary norming calculation from the
    remaining weak-* bipolar-density theorem.
*)
...
Section Polar.
  ...
  Hypothesis coord_contracting : forall j x, Rabs (coord j x) <= norm x.
  Hypothesis one_norming : forall x eps,
    0 < eps -> exists j, norm x - eps < Rabs (coord j x).

  Definition norming_polar (x : X) : Prop := forall j, Rabs (coord j x) <= 1.
  ...
  Theorem norming_polar_eq_closed_unit_ball : forall x,
    norming_polar x <-> closed_unit_ball x.
  Proof. intro x. split; [apply norming_polar_inside_unit_ball|apply unit_ball_inside_norming_polar]. Qed.
End Polar.
