(** OrderNeutralDescent.v -- quantitative resource core of authoritative v3 Theorem 7.4.

    This module proves the mechanism that makes proof genealogy order-neutral
    under geometric dyadic refinement: per-level O(M_n beta_n) payloads sum to
    O(M_n beta_n) at the finest relevant level, provided beta_n is
    nondecreasing.  It also proves the explicit O(2^n(n+1)) standard-rational
    specialization in refinement depth.

    Proposition 7.3 supplies the concrete per-level compiler/encoding bridge;
    Theorem 7.4 additionally constructs the represented limit name, attaches
    the actual proof DAG, converts target tolerance epsilon to m(epsilon), and
    accounts for verification/source-query costs.
*)
...
Proof.
  intros c f n. induction n; simpl; nia.
Qed.
...
Section DyadicGeometry.
  ...
  Proof.
    intro n. unfold M.
    pose proof (pow2_positive n). nia.
  Qed.
  ...
  Proof.
    ...
    rewrite sum_M_exact.
    unfold M.
    pose proof (pow2_positive n) as Hp.
    unfold pow2 in *.
    simpl Nat.pow.
    nia.
  Qed.
  ...
  Theorem genealogy_sums_to_finest_level : forall n,
    nsum_upto payload_bits n
      <= 2 * c_payload * M n * beta n.
  Proof.
    intro n.
    eapply Nat.le_trans.
    - apply payload_sum_bound_by_patch_sum.
    - pose proof (dyadic_patch_sum n).
      nia.
  Qed.
  ...
  Proof.
    intro n.
    pose proof (genealogy_sums_to_finest_level n) as Hg.
    pose proof (baseline_dominates n) as Hb.
    nia.
  Qed.
  ...
  Proof.
    intro n.
    pose proof (genealogy_sums_to_finest_level n) as Hg.
    specialize (beta_linear n) as Hb.
    unfold M in Hg.
    nia.
  Qed.
