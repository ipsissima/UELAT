(** PersistentGenealogy.v -- executable shared-history skeleton for H5.
    ...
    Theorem 7.4 requires H_{n+1} to contain H_n by shared reference, not by
    copying. Starting from one input leaf, this module appends backward rule
    nodes whose default reference is the previous sink. Node lists are persistent
    and grow by exactly the number of new steps.
*)
...
Section Genealogy.
  ...
  Proof. intros. unfold append_to_sink. apply append_rule_is_persistent. Qed.

  Theorem append_to_sink_adds_one : forall H rule payload,
    node_count (append_to_sink H rule payload) = S (node_count H).
  Proof. intros. unfold append_to_sink. apply append_rule_adds_one_node. Qed.
  ...
  Proof.
    ...
    - lia.
    - rewrite IH. rewrite append_to_sink_adds_one. simpl. lia.
  Qed.
  ...
  Proof.
    ...
    - destruct (IH (append_to_sink H r p)) as [tail Htail].
      unfold append_to_sink in Htail. simpl in Htail.
      rewrite Htail.
