(** ProofDAGBuilder.v -- append-only persistent proof genealogy.
    ...
    The descent compiler extends a history by shared reference rather than by
    copying previous nodes.  This file supplies the primitive operation used by
    that implementation: append one rule node whose references all point into
    the existing DAG.  The old node list is literally a prefix of the new one,
...
Section Builder.
  ...
  Proof.
    ...
    - intros i r rs p Hnth.
      ...
      + assert (Hi : i < length (dag_nodes H ++ [RuleNode rule refs payload])).
        { apply (proj1 (nth_error_Some _ i)).
  ...
  Proof.
    intros. unfold dag_prefix, append_rule. simpl.
    eexists. reflexivity.
  Qed.
