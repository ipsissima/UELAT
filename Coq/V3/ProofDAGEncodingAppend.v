(** ProofDAGEncodingAppend.v -- encoded size of append-only genealogy.
    ...
    Connects structural DAG sharing to the fixed binary encoding of Definition
    6.1. Appending one node adds its node encoding plus changed finite headers;
    the old payload is not copied.
*)
...
Section Encoding.
  ...
  Proof. intros xs ys. induction xs as [|x xs IH]; simpl; lia. Qed.
  ...
  Theorem append_rule_encoded_size_formula :
    ...
    dag_encoded_bitlength payload_bits rule_bits (append_rule H r refs p Hrefs)
  ...
  Proof.
    ...
    rewrite nodes_bitlength_app. simpl. lia.
  Qed.
  ...
  Proof.
    ...
    pose proof (nat_bitlength_positive (length (dag_nodes H))). nia.
  Qed.
  ...
  Proof.
    ...
    pose proof (append_rule_encoded_size_bound H r refs p Hrefs). nia.
  Qed.
End Encoding.
