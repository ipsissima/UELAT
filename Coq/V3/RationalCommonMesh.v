(** RationalCommonMesh.v -- executable arbitrary rational mesh refinement.
    ...
    RationalSobolev.v is exact on aligned meshes and RationalMeshRefinement.v
    proves one split exact. This file implements repeated insertion of rational
    breakpoints and preserves exact W12 self-energy.
*)
...
Definition split_piece_at (x : Q) (c : RationalPiece) : list RationalPiece :=
  if strictly_inside_piece x c then
    [ {| piece_left := piece_left c; piece_right := x; piece_poly := piece_poly c |};
      {| piece_left := x; piece_right := piece_right c; piece_poly := piece_poly c |} ]
  else [c].

Lemma split_piece_at_length : forall x c,
  length (split_piece_at x c) = 1 \/ length (split_piece_at x c) = 2.
Proof. intros x c. unfold split_piece_at. destruct (strictly_inside_piece x c); simpl; auto. Qed.

Definition piece_energy (c : RationalPiece) : Q :=
  qpoly_w12_inner_on (piece_left c) (piece_right c) (piece_poly c) (piece_poly c).

Fixpoint pieces_energy (cs : list RationalPiece) : Q :=
  match cs with [] => 0 | c :: rest => piece_energy c + pieces_energy rest end.

Lemma pieces_energy_app : forall xs ys,
  pieces_energy (xs ++ ys) == pieces_energy xs + pieces_energy ys.
Proof. intros xs ys. induction xs as [|x xs IH]; simpl; [ring|rewrite IH; ring]. Qed.
...
Proof.
  ...
  - simpl. unfold piece_energy. simpl. rewrite <- qpoly_w12_inner_split. ring.
  - simpl. ring.
Qed.
...
Proof.
  intros x cs. induction cs as [|c rest IH]; simpl; [reflexivity|].
  rewrite pieces_energy_app. rewrite split_piece_at_energy, IH. ring.
Qed.

Fixpoint refine_by_points (points : list Q) (cs : list RationalPiece) : list RationalPiece :=
  match points with [] => cs | x :: xs => refine_by_points xs (refine_pieces_at x cs) end.

Theorem refine_by_points_energy : forall points cs,
  pieces_energy (refine_by_points points cs) == pieces_energy cs.
Proof.
  intros points cs. induction points as [|x xs IH]; simpl; [reflexivity|].
  rewrite IH. apply refine_pieces_at_energy.
Qed.

Definition internal_breakpoints (cs : list RationalPiece) : list Q :=
  match cs with [] => [] | _ :: rest => map piece_left rest end.

Definition common_breakpoints (xs ys : list RationalPiece) : list Q :=
...
Proof. intros. unfold refine_pair_to_union. simpl. apply refine_by_points_energy. Qed.
...
Proof.
  intros x cs. induction cs as [|c rest IH]; simpl; [lia|].
  rewrite app_length.
