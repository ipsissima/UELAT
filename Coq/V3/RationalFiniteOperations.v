(** RationalFiniteOperations.v -- arbitrary-mesh finite operations for authoritative Lemma 5.2.

    Pairwise cell intersections form a finite rational common refinement.
    Addition, multiplication, rational scaling and rational restriction are
    exact finite Q operations, with no numerical quadrature or floating point.
*)
...
Fixpoint common_scale_pieces (a : Q) (xs : list RationalPiece) : list RationalPiece :=
  ...
  | [] => []
  | u :: rest =>
      {| piece_left := piece_left u; piece_right := piece_right u;
...
Proof.
  intros op u ys. induction ys as [|v rest IH]; simpl; [lia|].
...
Proof.
  intros op xs. induction xs as [|u rest IH]; intro ys; simpl; [lia|].
  rewrite app_length.
...
Proof.
  intros a b xs. induction xs as [|u rest IH]; simpl; [lia|].
...
Definition compute_exact_finite_w12
  ...
  {| efwa_inner := arbitrary_mesh_inner u v;
     ...
     efwa_inner_eq := eq_refl; efwa_sqdist_eq := eq_refl;
     efwa_add_eq := eq_refl; efwa_mul_eq := eq_refl;
...
Proof. intros u v. exists (compute_exact_finite_w12 u v). split; reflexivity. Qed.
