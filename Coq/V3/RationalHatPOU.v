(** RationalHatPOU.v -- constructive rational PWL partition core. *)
...
Definition left_hat_on_cell (a b x : Q) : Q := (b - x) / (b - a).
Definition right_hat_on_cell (a b x : Q) : Q := (x - a) / (b - a).

Theorem two_hat_partition_identity : forall a b x,
  ~ Qeq a b -> left_hat_on_cell a b x + right_hat_on_cell a b x == 1.
Proof.
  intros a b x Hab. unfold left_hat_on_cell, right_hat_on_cell.
  field. intro Hzero. apply Hab. lra.
Qed.

Theorem left_hat_nonnegative : forall a b x,
  (a < b)%Q -> (a <= x <= b)%Q -> (0 <= left_hat_on_cell a b x)%Q.
Proof.
  intros a b x Hab [Hax Hxb]. unfold left_hat_on_cell.
  apply Qle_shift_div_l; [lra|ring_simplify; lra].
Qed.
Theorem right_hat_nonnegative : forall a b x,
  (a < b)%Q -> (a <= x <= b)%Q -> (0 <= right_hat_on_cell a b x)%Q.
Proof.
  intros a b x Hab [Hax Hxb]. unfold right_hat_on_cell.
  apply Qle_shift_div_l; [lra|ring_simplify; lra].
Qed.
Theorem left_hat_at_most_one : forall a b x,
  (a < b)%Q -> (a <= x <= b)%Q -> (left_hat_on_cell a b x <= 1)%Q.
Proof.
  intros a b x Hab [Hax Hxb]. unfold left_hat_on_cell.
  apply Qle_shift_div_r; [lra|ring_simplify; lra].
Qed.
Theorem right_hat_at_most_one : forall a b x,
  (a < b)%Q -> (a <= x <= b)%Q -> (right_hat_on_cell a b x <= 1)%Q.
Proof.
  intros a b x Hab [Hax Hxb]. unfold right_hat_on_cell.
  apply Qle_shift_div_r; [lra|ring_simplify; lra].
Qed.
...
Theorem hat_slopes_cancel : forall a b,
  ~ Qeq a b -> left_hat_slope a b + right_hat_slope a b == 0.
Proof.
  intros a b Hab. unfold left_hat_slope, right_hat_slope.
  field. intro Hzero. apply Hab. lra.
Qed.
...
Theorem hat_cell_width_positive : forall c, (0 < hat_cell_width c)%Q.
Proof. intros c. unfold hat_cell_width. pose proof (hat_cell_positive c). lra. Qed.
Theorem hat_cell_derivative_bound_positive : forall c,
  (0 < hat_cell_derivative_bound c)%Q.
Proof. intro c. unfold hat_cell_derivative_bound. apply Qinv_lt_0_compat. apply hat_cell_width_positive. Qed.
...
Proof.
  intros c x Hx. apply two_hat_partition_identity.
...
Proof.
  intros c x Hx. pose proof (hat_cell_positive c) as Hpos.
  split; split.
