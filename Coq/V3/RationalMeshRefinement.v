(** RationalMeshRefinement.v -- exact common-mesh refinement core for v3 Lemma 5.2.
    ...
    polynomial pieces on the two subcells preserves the exact integral.
*)
...
Proof. intros. unfold qpoly_integral_between. apply qpoly_integral_between_from_additive. Qed.
...
Proof. intros. unfold qpoly_l2_inner_on. apply qpoly_integral_between_additive. Qed.
...
Proof. intros. unfold qpoly_w12_sqdist_on. apply qpoly_w12_inner_split. Qed.

Definition split_piece (mid : Q) (c : RationalPiece) : list RationalPiece :=
  [ {| piece_left := piece_left c; piece_right := mid; piece_poly := piece_poly c |};
    {| piece_left := mid; piece_right := piece_right c; piece_poly := piece_poly c |} ].
...
Definition split_pair_w12 (mid : Q) (u v : RationalPiece) : Q :=
  qpoly_w12_inner_on (piece_left u) mid (piece_poly u) (piece_poly v)
  + qpoly_w12_inner_on mid (piece_right u) (piece_poly u) (piece_poly v).
...
Proof.
  intros u v mid Hl Hr. unfold split_pair_w12.
  symmetry. apply qpoly_w12_inner_split.
Qed.

Record MeshRefinementCertificate := { refinement_points : list Q }.

Theorem one_split_exactness : forall p q a b c,
  qpoly_w12_sqdist_on a c p q
    = qpoly_w12_sqdist_on a b p q + qpoly_w12_sqdist_on b c p q.
Proof. apply qpoly_w12_sqdist_split. Qed.
