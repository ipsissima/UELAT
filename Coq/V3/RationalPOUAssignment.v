(** RationalPOUAssignment.v -- grouping rational nodal hats by cover patch.
    ...
    Lemma 5.4 assigns each closed vertex star to a containing cover interval.
    The standard nodal hats are grouped by that assignment; this file proves
    finite grouping preserves the cell-local partition identity.
*)
...
Lemma zero_buckets_length : forall M,
  length (zero_buckets M) = M.
Proof. induction M; simpl; congruence. Qed.

Lemma qsum_zero_buckets : forall M,
  qsum (zero_buckets M) == 0.
...
Proof.
  intros M entries Hrange.
  induction Hrange as [|[i v] rest Hi Hrest IH]; simpl.
...
Theorem bucketize_preserves_total : forall M entries,
  entries_in_range M entries ->
  qsum (bucketize M entries) == entry_value_sum entries.
Proof.
  intros M entries Hrange.
  induction Hrange as [|[i v] rest Hi Hrest IH]; simpl.
...
Definition cell_entries
    (A : StarAssignment) (k : nat) (a b x : Q) : list (nat * Q) :=
...
Proof.
  intros. unfold entries_in_range, cell_entries. simpl.
  repeat constructor; apply sa_vertex_in_range.
Qed.

Definition cell_patch_values
    (A : StarAssignment) (k : nat) (a b x : Q) : list Q :=
...
Proof.
  intros A k c x.
  apply grouped_hats_partition_cell.
