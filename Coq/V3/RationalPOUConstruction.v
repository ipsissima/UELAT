(** RationalPOUConstruction.v -- integrated constructor for authoritative Lemma 5.4.

    Given a rational subdivision fine enough that every closed vertex star lies
    in some interval of the finite rational cover, the assignment search,
    nodal hats and grouping yield a finite subordinate rational partition.
    Existence of a sufficiently fine subdivision remains the separate standard
    one-dimensional refinement fact stated in the manuscript proof.
*)
...
Definition assigned_patch (D : FineSubdivisionCover) (k : nat) : nat :=
  choose_covering_interval (fsc_cover D) (fsc_star D k)
    (fsc_star_covered D k).

Theorem assigned_patch_valid : forall D k,
  exists u,
    nth_error (fsc_cover D) (assigned_patch D k) = Some u
    /\ star_inside_interval (fsc_star D k) u.
Proof.
  intros D k.
  unfold assigned_patch.
  apply choose_covering_interval_valid.
Qed.
...
Proof.
  intros D k.
  destruct (assigned_patch_valid D k) as [u [Hnth Hinside]].
  apply (proj1 (nth_error_Some (fsc_cover D) (assigned_patch D k))).
  rewrite Hnth. discriminate.
Qed.

Lemma cover_length_positive : forall D,
  0 < length (fsc_cover D).
Proof.
  intro D.
  destruct (fsc_cover D) as [|u us] eqn:Hcover.
...
Proof.
  intros D k a b x Hab.
  apply grouped_hats_partition_cell. exact Hab.
Qed.

Theorem integrated_grouped_hats_partition_certified_cell : forall D k c x,
  qsum
    (cell_patch_values (integrated_star_assignment D) k
...
Proof.
  intros D k a b x.
  unfold cell_patch_values.
  apply bucketize_length.
