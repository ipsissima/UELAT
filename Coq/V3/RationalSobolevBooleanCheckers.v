(** RationalSobolevBooleanCheckers.v -- terminating finite checkers for authoritative Definition 5.1 / Proposition 5.3.

    The checker recomputes exact finite-stage squared W12 distances and verifies
    strict slack using decidable rational arithmetic. Soundness follows from
    fast-Cauchy tails; semantic strict slack yields an accepted finite witness.
*)
...
Definition qeqb (a b : Q) : bool := if Qeq_dec a b then true else false.
...
Record QApproxWitness := { qaw_stage : nat; qaw_sigma : Q }.
Record QDistanceWitness := { qdw_stage : nat; qdw_sigma : Q }.
...
Definition distance_check_Q
  ...
  andb (qltb (2 * qdyadic n) q)
    (andb
      (qeqb sigma (w12_sqdist (w12_stage nu n) (w12_stage mu n)))
      (qltb sigma ((q - 2 * qdyadic n) * (q - 2 * qdyadic n)))).
...
Section Soundness.
  ...
  Proof.
    ...
    rewrite HexactR in HsqR.
    rewrite (w12_sqdist_sound (P:=P) (w12_stage nu n) (w12_stage mu n)) in HsqR.
    pose proof (w12_stage_tail P nu n) as Hnu.
    pose proof (w12_stage_tail P mu n) as Hmu.
    pose proof (distance_nonnegative (w12_metric P)
      (w12_decode (w12_stage nu n)) (w12_decode (w12_stage mu n))) as Hfin0.
    assert (Hfin :
    ...
    { eapply Rle_trans.
      ...
      - eapply Rle_trans.
        ...
        + rewrite distance_symmetric with
            (x := w12_decode (w12_stage mu n)) (y := w12_name_value mu).
          lra. }
    lra.
  Qed.
End Soundness.
...
Section Completeness.
    ...
    - apply andb_true_iff. split.
      ...
      + apply qltb_true_iff. apply Rlt_Qlt.
        ...
        { eapply Rle_trans.
          ...
          - rewrite distance_symmetric with
              (x := w12_decode (w12_stage nu n)) (y := w12_name_value nu).
            unfold dtrue. lra. }
        unfold dtrue in *. nra.
  Qed.
  ...
  Proof.
    ...
    unfold distance_check_Q. simpl. apply andb_true_iff. split.
    - apply qltb_true_iff. apply Rlt_Qlt.
    ...
    - apply andb_true_iff. split.
      ...
      + apply qltb_true_iff. apply Rlt_Qlt.
        rewrite Q2R_mult. repeat rewrite Q2R_minus.
        repeat rewrite Q2R_mult. repeat rewrite qdyadic_real.
        change (Q2R (2 : Q)) with 2%R.
        rewrite (w12_sqdist_sound (P:=P) (w12_stage nu n) (w12_stage mu n)).
        pose proof (w12_stage_tail P nu n) as Hnu.
        pose proof (w12_stage_tail P mu n) as Hmu.
        pose proof (distance_nonnegative (w12_metric P)
          (w12_decode (w12_stage nu n)) (w12_decode (w12_stage mu n))) as Hfin0.
