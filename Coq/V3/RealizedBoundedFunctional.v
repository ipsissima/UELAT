(** RealizedBoundedFunctional.v -- strong computability interface for authoritative Section 3.

    A realized bounded functional carries both its semantic bounded-linear data
    and an executable rational Type-2 approximation program on source fast names.
    This prevents Theorem 3.2 from silently weakening “computable functional”
    to a merely semantic bounded functional.
*)
  ...
  rbf_realize_correct : forall (x : CoreNamedPoint B) n,
    Rabs
      (Q2R (rbf_realize (core_named_name x) n)
       - rbf_apply (core_named_value x)) <= dyadic n
}.
