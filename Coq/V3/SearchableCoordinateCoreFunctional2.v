(** SearchableCoordinateCoreFunctional2.v -- executable dense-core functional from an admissible coordinate point.

    The direct value of a core code is its normalization scale times the
    coordinate at the searchable enumeration index. Span-graph theorems prove
    this value is presentation-independent, bounded and rational-linear.
*)

From Coq Require Import Reals QArith Qreals Lra Ring Field.
From UELAT.V3 Require Import
  CertificateEnrichment ComputableBanach SearchableCore
  DualBallCoordinates CoordinateDualBall CoordinateFunctionalGraph.
...
Section DirectCoreFunctional.
  ...
  Definition direct_core_value (p : core_code B) : R :=
    Q2R (core_scale_factor B p) * cdb_coordinates B a (core_index S p).

  Lemma normalized_index_for_code : forall p,
    normalized_core B (core_index S p) = core_decode (normalized_core_code B p).
  Proof. intro p. unfold normalized_core. rewrite core_index_correct. reflexivity. Qed.
  ...
  Proof.
    ...
    replace (Q2R (core_scale_factor B p) * / Q2R (core_scale_factor B p)) with 1
      by (field; lra).
    apply cb_scale_one.
  Qed.
  ...
  Proof.
    intro p.
    pose proof (coordinate_span_graph_generator B a (core_index S p)) as Hg.
    pose proof
      (coordinate_span_graph_scale_Q B a (core_scale_factor B p)
        (normalized_core B (core_index S p))
        (cdb_coordinates B a (core_index S p)) Hg) as Hscaled.
    unfold direct_core_value.
    rewrite normalized_index_for_code in Hscaled.
    rewrite rescale_normalized_code in Hscaled. exact Hscaled.
  Qed.

  Theorem direct_core_value_bounded : forall p,
    Rabs (direct_core_value p) <= cb_norm B (core_decode p).
  Proof. intro p. eapply coordinate_span_graph_bounded. apply direct_core_value_in_span_graph. Qed.

  Lemma zero_span_graph : CoordinateSpanGraph B a (core_decode (core_zero B)) 0.
  Proof.
  ...
  Proof.
    ...
    - apply zero_span_graph.
  Qed.

  Theorem direct_core_value_add : forall p q,
    direct_core_value (core_add B p q) = direct_core_value p + direct_core_value q.
  Proof.
    intros p q. eapply coordinate_span_graph_functional.
    - apply direct_core_value_in_span_graph.
    - rewrite core_add_sound. apply coordinate_span_graph_add;
        apply direct_core_value_in_span_graph.
  Qed.
  ...
  Proof.
    intros c p. eapply coordinate_span_graph_functional.
    - apply direct_core_value_in_span_graph.
    - rewrite core_scale_sound. apply coordinate_span_graph_scale_Q.
      apply direct_core_value_in_span_graph.
  Qed.
  ...
  Theorem admissible_coordinate_gives_bounded_rational_core_functional :
    ...
    /\ (forall p q, direct_core_value (core_add B p q)
        = direct_core_value p + direct_core_value q)
    /\ (forall c p, direct_core_value (core_scale B c p)
        = Q2R c * direct_core_value p).
  Proof.
    repeat split.
