(** SlackCollapse.v -- conditional extensional collapse, v3 Section 2.2.

    The manuscript defines the evidence pseudometric as the infimum of accepted
    distance bounds.  This file proves that statement literally in its
    order-theoretic form: the represented metric is the greatest lower bound of
    the accepted evidence bounds.  No heavy real-infimum library is required;
    a greatest-lower-bound predicate is defined directly and proved from
    checker soundness plus strict-slack completeness.
*)
...
Section Collapse.
  ...
  Proof.
    intros [w Hw].
    pose proof (dist_sound E _ _ _ _ Hw) as [_ H].
    exact H.
  Qed.
  ...
  Proof.
    intro Hlt.
    destruct (Hcomplete nu mu q Hlt) as [w Hw].
    now exists w.
  Qed.

  Theorem accepted_bounds_characterize_distance
      (Hcomplete : StrictSlackComplete)
      (nu mu : name X) :
    (forall q, EvidenceBound nu mu q ->
       distance (decode_name nu) (decode_name mu) <= q)
    /\
    (forall r,
       distance (decode_name nu) (decode_name mu) < r ->
       exists q, EvidenceBound nu mu q /\ q < r).
  Proof.
    split.
    - intros q Hq. now apply evidence_bounds_are_sound.
    - intros r Hr.
      exists ((distance (decode_name nu) (decode_name mu) + r) / 2).
      split.
      + apply strict_slack_supplies_every_larger_bound; try exact Hcomplete.
        lra.
      + lra.
  Qed.

  (** Manuscript Theorem 2.8: the evidence infimum is exactly the represented
      analytic distance.  [GreatestLowerBound] is the defining universal
  ...
  Proof.
    intro Hz.
    rewrite <- Hz.
    apply evidence_distance_is_represented_distance.
    exact Hcomplete.
  Qed.
