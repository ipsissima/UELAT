(** SobolevPUFEMAnalyticInterface.v -- standard analytic boundary for authoritative Theorem 5.6.

    Product rules, multiplier estimates and bounded-overlap inequalities are
    standard analysis, not certificate-specific novelty. This file isolates
    their numerical consequences per overlap incidence and derives the complete
    localized PUFEM bound from them.
*)
...
Record AnalyticIncidence (Cinf : R) := {
  ...
  ai_product_rule_bound :
    ai_deriv_term_sq <=
      2 * (Ldelta0_sq ai_defect + Cinf^2 * delta1_sq ai_defect)
}.
...
Fixpoint analytic_defects {Cinf : R}
    (xs : list (AnalyticIncidence Cinf)) : list PairwiseDefectDatum :=
  match xs with [] => [] | x :: rest => ai_defect x :: analytic_defects rest end.

Fixpoint sum_l2_terms {Cinf : R}
    (xs : list (AnalyticIncidence Cinf)) : R :=
  match xs with [] => 0 | x :: rest => ai_l2_term_sq x + sum_l2_terms rest end.

Fixpoint sum_deriv_terms {Cinf : R}
    (xs : list (AnalyticIncidence Cinf)) : R :=
...
Proof.
  ...
  - nra.
  - pose proof (ai_multiplier_bound x).
    unfold sum_delta0_sq in *; simpl in *. nra.
Qed.
...
Lemma analytic_sum_l2_nonnegative : forall Cinf xs,
  0 <= sum_l2_terms xs.
Proof.
...
Lemma analytic_sum_deriv_nonnegative : forall Cinf xs,
  0 <= sum_deriv_terms xs.
Proof.
  intros Cinf xs. induction xs as [|x rest IH]; simpl; [lra|].
...
Section Derivation.
  ...
  Proof.
    ...
    - eapply Rle_trans; [apply boa_l2_overlap|].
      ...
      apply analytic_sum_l2_bound.
    - eapply Rle_trans; [apply boa_deriv_overlap|].
      apply Rmult_le_compat_l; [exact Hkappa|].
      apply analytic_sum_deriv_bound.
  Defined.
  ...
  Proof.
    exact (cld_total_defect_bound compiled_analytic_localized_defect).
  Qed.
