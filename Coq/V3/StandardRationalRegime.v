(** StandardRationalRegime.v -- authoritative Corollary 7.5.

    At dyadic target depth s, the precision-relevant refinement level scales
    like s/(r-1); the factor 2^m is the discrete proxy for
...
Definition inverse_alpha_proxy (r s : nat) : nat :=
  pow2 (precision_level r s).
Definition logarithmic_proxy (r s : nat) : nat :=
  S (precision_level r s).
...
Section Corollary.
  ...
  Theorem standard_rational_precision_bound : forall r s,
    2 <= r ->
    nsum_upto payload_bits (precision_level r s)
      <= 2 * c_payload * M0 * beta_factor
           * inverse_alpha_proxy r s * logarithmic_proxy r s.
  Proof.
    intros r s Hr. unfold inverse_alpha_proxy, logarithmic_proxy.
    eapply standard_rational_depth_bound; eauto.
  Qed.

  Theorem standard_rational_relative_to_baseline : forall r s,
    2 <= r ->
    nsum_upto payload_bits (precision_level r s)
      <= 2 * c_payload * base_factor
           * ordinary_bits (precision_level r s).
  Proof. intros r s Hr. eapply order_neutral_relative_to_baseline; eauto. Qed.
End Corollary.
