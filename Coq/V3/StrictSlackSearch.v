(** StrictSlackSearch.v -- executable finite-stage search for v3 Proposition 2.5.
    ...
    Strict metric claims over effective Cauchy names are semidecided by checking
    finite stages until a rational upper enclosure crosses the requested bound.
    For decidable predicates on natural numbers, Coq's constructive epsilon
    library turns an eventual-witness proof into an axiom-free terminating
    search.  Thus no externally supplied numerical search bound is required:
    the remaining analytic obligation is precisely to prove that some finite
    stage succeeds when semantic strict slack is available.
*)
...
Proof.
  intros test fuel.
  induction fuel as [|fuel IH]; intros n H.
  - simpl in H. destruct (test 0) eqn:Ht; inversion H; subst; assumption.
  - simpl in H.
    destruct (first_true_upto test fuel) eqn:Hprev.
...
Proof.
  intros test fuel.
  induction fuel as [|fuel IH]; intros witness Hle Htrue.
  - assert (witness = 0) by lia. subst.
    simpl. rewrite Htrue. eauto.
  - simpl.
    destruct (first_true_upto test fuel) eqn:Hprev.
    + eauto.
...
Proof.
  intros test eventually.
  unfold first_true_index.
...
Proof.
  intro S.
  destruct (stage_bound_success S) as [w [Hw Htest]].
  unfold run_slack_search.
  now apply first_true_upto_complete with (witness := w).
Qed.
