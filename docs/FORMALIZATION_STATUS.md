...
3. `Print Assumptions <theorem>` has been captured and reviewed, and
   the output committed under `docs/assumptions/<stem>.txt`.
...
5. The `checked_at` column names the exact commit at which the audit
   ran green.

`CHECKED-RESTRICTED` requires the same discipline PLUS a documented
delta from the paper theorem (extra hypothesis, narrower quantifier,
etc.).

`CHECKED-ANALYTIC-CORE` requires `Print Assumptions` capture whenever
the lemma is advertised as reusable checked mathematical content.

`DEFINITION-EXACT` requires successful compilation and correspondence
inspection only; no theorem-side assumption audit is meaningful — a
definition has no `Print Assumptions` in the interesting sense.

The `checked_at` column carries the exact commit hash the audit ran
against for advanced-status rows. `—` means the row has not yet
reached any CHECKED-* / DEFINITION-EXACT status against a specific
audited commit.
...
| v3 paper result | section | Rocq theorem | module | status | assumptions | notes | checked_at |
| --- | --- | --- | --- | --- | --- | --- | --- |
| §2 normalized proof-tree convention | §2 | `V3_EvidenceSyntax.Spine`, `sp_nil`, `sp_app`, `sp_bound`, `sp_transport` | `Coq/V3/EvidenceSyntax.v` | DEFINITION-EXACT | — | The paper's flattened spine with reflexivity as unit. Endpoint-indexed, so non-composable spines are untypeable and Θ_T sees both endpoints. Strict unitality/associativity (`sp_app_nil_l/_r`, `sp_app_assoc`), additive bounds (`sp_bound_app`), and the two Θ_T laws (`sp_transport_nil/_app`) are proved as Leibniz equalities. Bounds in `Qc` for that reason. Gaps recorded, not claimed: symmetry and weakening of spines. | — |
| Def 2.1 Approximation presentation | §2 | `V3_Presentation.Presentation` record | `Coq/V3/Presentation.v` | IN-PROGRESS | — | Semantic checker core of Def 2.1. The presentation now supplies only the PRIMITIVE `DistLeaf` checker; whole-claim distance certification is DERIVED (`certified_dist`) and whole-spine soundness is the proved bridge `spine_sound`, so the triangle rule is a theorem rather than an assumed closure rule. Bounds are canonical `Qc`. Still missing from Def 2.1: effective enumerability of `CodeF`, dense range of `rhoF`, and the represented subdomain `D_F` with `deltaF` surjective onto it. Recorded restriction: a presentation whose only natural checker is whole-claim and not leaf-decomposable would not fit this record. | — |
...
| Def 3.1 Proof-relevant evidence category | §3 | `V3_Evidence.EvidenceObject`, `V3_Evidence.EvidenceMorphism`, `V3_Evidence.id_evidence`, `V3_Evidence.comp_evidence` | `Coq/V3/Evidence.v` | DEFINITION-EXACT | `EvidenceClosure` (symmetry, weakening, mixed rule only) | A morphism IS a normalized `Spine` between the objects' names, so endpoints live in the type and no endpoint proof is stored. The category laws `comp_evidence_id_l` / `comp_evidence_id_r` / `comp_evidence_assoc` are now proved as LEIBNIZ equalities — this discharges the deferral recorded in the previous revision. Reflexivity and the triangle rule are theorems, not closure fields. | — |
...
| Prop 3.3 Soundness of evidence metric (lower-bound half) | §3 | `V3_MetricReflection.prop_3_3_lower_bound` (+ corollary `lawvere_bounds_analytic`) | `Coq/V3/MetricReflection.v` | CHECKED-RESTRICTED | stdlib classical reals: `ClassicalDedekindReals.sig_forall_dec`, `FunctionalExtensionality.functional_extensionality_dep` | The lower-bound direction: every accepted rational bound dominates `distF`. Full Prop 3.3 also includes surjectivity of the certifiable-subset image, deferred to a downstream module that will invoke Prop 2.4. | — |
...
| Thm 4.4 Extensional collapse (first equation) | §4 | `V3_MetricReflection.extensional_collapse` | `Coq/V3/MetricReflection.v` | CHECKED-RESTRICTED | rational density hypothesis `Q_dense_R`; stdlib classical reals: `ClassicalDedekindReals.sig_forall_dec`, `FunctionalExtensionality.functional_extensionality_dep` | Proves `is_lawvere_dist c d (distF …)` under distance adequacy AND a `Q dense in R` hypothesis passed as a Section variable. The hypothesis is a true fact of ℝ; a follow-up commit will discharge it inline. Quotient-object statement (isometry to certifiable subset, separated reflection universal property) not yet formalized. | — |
| Def 5.1 Certifiably realizable Lipschitz map | §5 | `V3_RealizableMap.RealizableMap` | `Coq/V3/RealizableMap.v` | IN-PROGRESS | evidence regularity (Def 4.3) and the §2 closure rules, for `rm_app_transport` only | Record now has the paper's FOUR clauses. Clause 1 carries a stored finite derivation (`rm_lip_store` / `rm_lip_apply` / `rm_lip_apply_ok`) alongside the analytic Prop. Clause 4's Θ_T is DEFINED from that derivation (`rm_theta_prim`, `rm_theta`) with its strict laws proved (`rm_theta_id`, `rm_theta_comp`). The extra fifth field `rm_app_promote` is REMOVED and derived instead as `rm_app_transport`. Still IN-PROGRESS because object- and morphism-level T_* are not yet constructed. | — |
