...
| label | meaning |
| --- | --- |
| `CHECKED-EXACT` | The named Rocq theorem has the same statement, at the same strength (same hypotheses, same conclusion), as the paper theorem, and lives on a `coqchk`-clean dependency path in the CI-built module set. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Def 2.1 Approximation presentation | §2 | `ApproximationPresentation` record | `Coq/V3/Presentation.v` | IN-PROGRESS | — | Layer-A skeleton committed; carriers of finite codes / names / checkers, soundness laws for `AppCheck` and `DistCheck`. |
...
| Thm 5.2 Generic lifting | §5 | — | — | PAPER-ONLY | — | Replaces the withdrawn probes–models adjunction as v3's categorical vehicle. |
